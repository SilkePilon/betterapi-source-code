...
driver = Webdriver(webdriver_path="/test/usr/bin/chromedriver", hide=True, headless=True).driver
...
app.mount("/test/about", StaticFiles(directory="static",html = True), name="static")
...
def sendmessagetochat(message, request):
    try:
        ...
        # subprocess.call(["taskkill", "/test/im", "chromedriver.exe"],shell=True)
...
@app.get("/test/status", response_class=HTMLResponse)
